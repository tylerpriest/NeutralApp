...
import { UIComponent, ComponentLocation, LayoutConfig, ResponsiveBreakpoint } from '../../../shared/types';
...
describe('LayoutManager', () => {
  ...
  describe('onBreakpointChange', () => {
    it('should register breakpoint change listener', () => {
      ...
      // Mock getCurrentBreakpoint to return a different breakpoint to trigger change
      const originalGetCurrentBreakpoint = layoutManager.getCurrentBreakpoint;
      jest.spyOn(layoutManager, 'getCurrentBreakpoint').mockReturnValue(ResponsiveBreakpoint.MOBILE);

      // Simulate window resize
      layoutManager.handleResize();

      expect(mockCallback).toHaveBeenCalledWith(ResponsiveBreakpoint.MOBILE);

      // Restore and cleanup
      layoutManager.getCurrentBreakpoint = originalGetCurrentBreakpoint;
      unsubscribe();
    });
