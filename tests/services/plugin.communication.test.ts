import { PluginEventBus } from '../../src/services/plugin.event.bus';
import { PluginStorageManager } from '../../src/services/plugin.storage.manager';
import {
  PluginEvent,
  EventHandler,
  SecurityViolation,
  ...
  describe('PluginEventBus', () => {
    ...
    describe('Plugin Lifecycle Integration', () => {
      ...
      it('should revoke permissions when plugin is unloaded', () => {
        eventBus.grantPermission('temp-plugin', 'admin-events');
        expect(eventBus.validateEventPermissions('temp-plugin', 'admin-events')).toBe(true);

        eventBus.unloadPlugin('temp-plugin');

        expect(eventBus.validateEventPermissions('temp-plugin', 'admin-events')).toBe(false);
      });
    });
  });
