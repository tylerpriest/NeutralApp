...
import { ComponentFailureHandler } from '../../src/services/component-failure-handler';
import { FallbackLogger } from '../../src/services/fallback-logger';
import { DeveloperNotificationService } from '../../src/services/developer-notification.service';
import { LoggingService } from '../../src/services/logging.service';
import { ErrorSeverity, LogLevel } from '../../src/interfaces/logging.interface';
...
describe('ErrorRecoveryService', () => {
  ...
  describe('Graceful Degradation for Component Failures', () => {
    it('should handle component failure gracefully without affecting other components', async () => {
      ...
      const context = { component: componentId, severity: ErrorSeverity.MEDIUM };
      ...
      expect(mockLoggingService.logError).toHaveBeenCalledWith(error, expect.objectContaining({
    ...
    it('should maintain system functionality when non-critical components fail', async () => {
      ...
      expect(systemHealth.status).toBe('degraded'); // Not 'down'
      expect(systemHealth.criticalSystemsOperational).toBe(true);
      expect(systemHealth.failedComponents).toHaveLength(3);
    });
  ...
  describe('Fallback Logging System', () => {
    ...
    it('should preserve critical log entries in fallback storage', () => {
      ...
      expect(mockFallbackLogger.log).toHaveBeenCalledWith({
  ...
  describe('Developer Notification System', () => {
    ...
    it('should send email notifications for high severity errors', async () => {
      ...
      expect(mockDeveloperNotificationService.sendEmail).toHaveBeenCalledWith({
        subject: 'High Severity Error: Plugin security violation',
        body: expect.stringContaining('suspicious-plugin'),
        recipients: expect.arrayContaining(['security@company.com']),
        priority: 'high'
      });
    });
    ...
    it('should batch non-critical notifications to avoid spam', async () => {
      ...
      expect(mockDeveloperNotificationService.notifyDeveloper).toHaveBeenCalledTimes(1);
      expect(mockDeveloperNotificationService.notifyDeveloper).toHaveBeenCalledWith({
        type: 'batch',
  ...
  describe('Error Recovery Integration', () => {
    ...
    it('should provide system health status including recovery state', async () => {
      ...
      expect(healthStatus).toEqual({
        status: expect.oneOf(['healthy', 'degraded', 'critical']),
    ...
    it('should handle cascading failures by isolating components', async () => {
      ...
      const healthStatus = await errorRecoveryService.getSystemHealth();
      expect(healthStatus.failedComponents).toHaveLength(3);
      expect(mockComponentFailureHandler.markComponentUnhealthy).toHaveBeenCalledTimes(3);
    });
  });
  ...
  describe('Error Recovery Performance', () => {
    ...
    it('should limit resource usage during error storms', async () => {
      ...
      expect(mockLoggingService.logError).toHaveBeenCalledTimes(expect.toBeLessThan(100));
    });
  });
}); 